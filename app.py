...
anthropic_client = anthropic.Anthropic(api_key=ANTHROPIC_API_KEY, timeout=15.0)
...
def end_period(user_id, end_date=None):
    if not end_date:
    ...
    if p:
        conn.execute("UPDATE periods SET end_date = ? WHERE id = ?", (end_date, p["id"]))
        conn.commit()
    conn.close()
    return p, end_date
...
- บอกว่าเมนส์มาแล้ว / มาแล้ว / มาวันนี้ → {{{{"action": "start_period", "date": "YYYY-MM-DD"}}}}
  ...
  คำนวณวันแรกในหัว แล้ว return เป็น YYYY-MM-DD จริงๆ เช่น "2025-04-23" ห้าม return code
  "วันนี้เป็นวันที่ N" = วันแรกคือ N-1 วันก่อน, "มา N วันแล้ว" = วันแรกคือ N วันก่อน
...
ถ้า message มีทั้ง start และ end ในประโยคเดียว เช่น "มาวันที่ 6 แล้วหมดวันที่ 11" ให้ return action end_period พร้อม start_date และ date เช่น {{"action": "end_period", "start_date": "2026-04-06", "date": "2026-04-11"}}
...
หลังจาก end_period สำเร็จแล้ว ห้ามถามเรื่องปริมาณเลือดอีก
ถ้า message มีทั้ง start และ end ในประโยคเดียว ให้ return {{"action": "end_period", "start_date": "YYYY-MM-DD", "date": "YYYY-MM-DD"}}
วันนี้คือ {datetime.now().strftime("%Y-%m-%d")}"""
...
def process_claude_response(user_id, response_text):
    ...
    try:
        ...
        if action == "start_period":
            ...
            start_date_override = data.get("start_date", None)
            if start_date_override:
                try:
                    datetime.strptime(start_date_override, "%Y-%m-%d")
                    conn = get_db()
                    conn.execute("UPDATE periods SET end_date = ? WHERE user_id = ? AND end_date IS NULL",
                                 (start_date_override, user_id))
                    conn.execute("INSERT INTO periods (user_id, start_date) VALUES (?, ?)",
                                 (user_id, start_date_override))
                    conn.commit()
                    conn.close()
                except ValueError:
                    pass
            ...
            try:
                ...
                end_date = datetime.now().strftime("%Y-%m-%d")
            result = end_period(user_id, end_date)
            period, actual_end = result if isinstance(result, tuple) else (result, end_date)
            if period:
                start = datetime.strptime(period["start_date"], "%Y-%m-%d")
                end = datetime.strptime(actual_end, "%Y-%m-%d")
                ...
                today = datetime.now().strftime("%Y-%m-%d")
                date_note = f" (บันทึกวันที่ {end.strftime('%d/%m/%Y')})" if actual_end != today else ""
                if is_bf:
                    reply = f"บันทึกแล้วนะ!{date_note} รอบนี้แฟนมา {days} วัน\nรอบหน้าน่าจะประมาณ {next_str}"
                    if days_left and days_left > 0:
                        reply += f" (อีก {days_left} วัน)"
                    reply += "\n\nช่วงนี้แฟนน่าจะรู้สึกดีขึ้นแล้วนะ ดูแลเขาต่อด้วยนะ 💙"
                else:
                    reply = f"บันทึกแล้วนะ ✨{date_note} รอบนี้มา {days} วัน\nรอบหน้าน่าจะประมาณ {next_str}"
                    if days_left and days_left > 0:
                        reply += f" (อีก {days_left} วัน)"
...
def handle_message(event):
    ...
    # โหมดแฟน
    ...
    elif user_message in ["ใช่", "ใช่ค่ะ", "ใช่ครับ", "หมดวันนี้", "ใช่วันนี้"]:
        latest = get_latest_period(user_id)
        if latest and latest["end_date"] is None:
            today = datetime.now().strftime("%Y-%m-%d")
            reply_text = process_claude_response(user_id, '{"action": "end_period", "date": "' + today + '"}')
        else:
            try:
        ...
        # คำสั่งตรงๆ ที่ไม่ต้องผ่าน Claude
            ...
            has_end = any(w in user_message for w in ["หมด", "หยุด", "ไม่มาแล้ว"])
            has_date = any(w in user_message for w in ["วันที่", "เมื่อวาน", "วันก่อน", "สองวัน", "สามวัน"])
            if has_end or has_date:
                try:
                    claude_response = chat_with_claude(user_id, user_message)
                    reply_text = process_claude_response(user_id, claude_response)
                except Exception as e:
